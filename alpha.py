...
def generate_bark(prompt, input_wav_file):
...
with gr.Blocks(theme=gr.themes.Base()) as app:
    with gr.Tabs():
        ...
        with gr.TabItem("Instant Cloning (BARK)"):
            with gr.Column():
                prompt = gr.Textbox(label="Prompt:")
                audio = gr.Audio(source="upload", type="filepath", label="Voice You Want To Clone (20 seconds):")
                bark = gr.Button("Bark")
            with gr.Column():
                clone = gr.Audio(label="Generation:")
            bark.click(fn=generate_bark,inputs=[prompt,audio],outputs=[clone])
            try:
                if tab_faq == "常见问题解答":
